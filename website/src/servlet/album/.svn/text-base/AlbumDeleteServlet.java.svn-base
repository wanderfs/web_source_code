...
@WebServlet(name = "AlbumDeleteServlet", urlPatterns = {"/album_delete"})
public class AlbumDeleteServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (user.deleteChannel(album))
            {
                sb.append("OK");
            }
            else {
                sb.append(album.getAdministratorid());
            }
        } finally {
            out.write("<deletereply>" + sb + "</deletereply>");
            out.close();
        }
    }
